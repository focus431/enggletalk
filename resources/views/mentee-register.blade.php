            ...
            <div class="row align-items-center justify-content-center">
              <div class="col-md-12 col-lg-6 login-right">
                <div class="login-header">
                  <h3>{{ __('messages.mentee_register') }} <a href="mentor-register">{{ __('messages.not_a_mentee') }}</a>
                  </h3>
                </div>
                ...
                <form id="mentee-register-form">
                  ...
                  <div class="row form-row social-login">
                    <div class="col-12 mb-2">
                      <a href="{{ route('google.login') }}" class="btn btn-google btn-block w-100">
                        <i class="fab fa-google me-1"></i> {{ __('messages.login_with_google') }}
                      </a>
                    </div>
                    <div class="col-12 mb-2">
                      <a href="{{ route('microsoft.login', ['role' => 'mentee']) }}"
                  ...
                  <input type="hidden" name="g-recaptcha-response" id="g-recaptcha-response-mentee">
                </form>
    ...
    form.addEventListener('submit', function(e) {
        ...
        grecaptcha.ready(function() {
            grecaptcha.execute('{{ config('recaptcha.api_site_key') }}', {action: 'register'})
                .then(function(token) {
                    document.getElementById('g-recaptcha-response-mentee').value = token;
                    const formData = new FormData(form);
                    formData.append('browserLang', navigator.language || navigator.userLanguage);
                    
                    fetch('/mentee-register', {
                        method: 'POST',
                        headers: {
